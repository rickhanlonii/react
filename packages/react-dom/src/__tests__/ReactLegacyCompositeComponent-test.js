...
describe('ReactLegacyCompositeComponent with Activity', () => {
  beforeEach(() => {
    jest.resetModules();
    React = require('react');
    ReactDOM = require('react-dom');
  ...
  it('should pass context to children when not owner', async () => {
    ...
    await act(() => {
      root.render(
        <Activity mode="visible">
          <Parent ref={current => (component = current)} />
        </Activity>,
      );
    });
    assertConsoleErrorDev([
    ...
    expect(container.innerHTML).toBe('<div style="display: none;">bar</div>');
    ...
    expect(container.innerHTML).toBe('<div style="">bar</div>');
  });
  ...
  it('should pass context when re-rendered for static child', async () => {
    ...
    expect(parentInstance).toBe(null);
    expect(childInstance.context).toEqual({foo: 'bar', flag: true});
  ...
  // @gate !disableLegacyContext
  it('should pass context when re-rendered for static child (activity around middle)', async () => {
    let parentInstance = null;
    let childInstance = null;
    ...
    // set context while child is hidden
    await act(() => {
      parentInstance.setState({flag: false});
    });

    expect(parentInstance.state.flag).toBe(false);
    expect(childInstance.context).toEqual({foo: 'bar', flag: true});

    // now reveal the child
    await act(() => {
      root.render(
        <Parent ref={current => (parentInstance = current)}>
          <Activity mode="visible">
  ...
  // @gate !disableLegacyContext
  it('should pass context when re-rendered for static child (activity around child)', async () => {
    let parentInstance = null;
    let childInstance = null;
  ...
  it('should pass context when re-rendered for static child within a composite component', async () => {
    ...
    await act(() => {
      root.render(
        <Wrapper mode="visible" ref={current => (wrapper = current)} />,
      );
    });
    ...
    await act(() => {
      root.render(
        <Wrapper mode="hidden" ref={current => (wrapper = current)} />,
      );
    });

    expect(wrapper.parentRef.current).toEqual(null);
    expect(wrapper.childRef.current).toEqual(null);
  ...
  // @gate !disableLegacyContext
  it('should pass context when re-rendered for static child within a composite component (activity for child)', async () => {
    class Parent extends React.Component {
      static childContextTypes = {
        flag: PropTypes.bool,
      };
    ...
    // Update <Parent /> while <Child /> is hidden
    await act(() => {
      wrapper.parentRef.current.setState({flag: true});
    });
  ...
  it('should pass context transitively', async () => {
    ...
    class Parent extends React.Component {
      ...
      render() {
        parentInstance = this;
        const {parentMode, childMode} = this.props;

        return (
          <Activity mode={parentMode}>
            <Child childMode={childMode} />
          </Activity>
        );
      }
    }
    ...
    class Child extends React.Component {
      ...
      render() {
        childInstance = this;
        const {childMode} = this.props;
        return (
          <Activity mode={childMode}>
            <Grandchild />
          </Activity>
        );
      }
    ...
    class Grandchild extends React.Component {
      ...
      render() {
        ...
        return (
          <div>
            {this.context.flag} {this.context.depth}
          </div>
        );
      }
    }
    ...
    const root = ReactDOMClient.createRoot(container);
    await act(() => {
      root.render(<Parent parentMode="visible" childMode="visible" />);
    });
    ...
    expect(childInstance.context).toEqual({flag: true, depth: 0});
    expect(grandchildInstance.context).toEqual({flag: true, depth: 1});

    // Hide child
    await act(() => {
      root.render(<Parent parentMode="hidden" childMode="visible" />);
    });

    expect(childInstance.context).toEqual({flag: true, depth: 0});
    expect(grandchildInstance.context).toEqual({flag: true, depth: 1});

    // Update parent context while child is hidden
    await act(() => {
      parentInstance.setState({flag: false});
    });

    expect(childInstance.context).toEqual({flag: false, depth: 0});
    expect(grandchildInstance.context).toEqual({flag: true, depth: 1});

    // Update child context while child is hidden
    await act(() => {
      childInstance.setState({flag: false});
    });

    expect(childInstance.context).toEqual({flag: false, depth: 0});
    expect(grandchildInstance.context).toEqual({flag: false, depth: 1});

    // hide grandchild
    await act(() => {
      root.render(<Parent parentMode="hidden" childMode="hidden" />);
    });
    ...
    await act(() => {
      root.render(<Parent parentMode="visible" childMode="hidden" />);
    });

    // Update parent and child context while grandchild is hidden
    await act(() => {
      childInstance.setState({flag: true});
      parentInstance.setState({flag: true});
    });
  ...
  it.only('should surface the “Unexpected context” bug', async () => {
    ...
    function Flipper() {
      ...
      if (count === 0) {
        count++;
        // This will cause the first commit to be popped.
        throw new Error('Flipping');
      }
      // Flip as soon as the first commit finishes.
      // React.useLayoutEffect(() => {
      //   if (showA) {
      //     // This schedules a brand-new render *while* React is still unwinding
      //     // from the first one, so the first push never gets popped.
      //     setShowA(false);
      //   }
      // }, [showA]);

      return <div>{showA ? <span>A</span> : <span>B</span>}</div>;
    }
    ...
    }).rejects.toThrow('There was an error during concurrent');
