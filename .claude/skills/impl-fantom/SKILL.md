...
- `packages/renderer/` must exist (reconciler host config)
- `packages/bridge/` must exist (bridge JS side)
- `ios/Sources/ReactDomNative/Bridge/` must exist (Swift bridge code)
...
Move UIKit-independent shadow tree code from `ios/Sources/ReactDomNative/Bridge/` into a new `ios/Sources/ShadowTree/` library target. The ShadowTree module must have **zero UIKit imports** so it compiles on macOS.

**Files to create in `ios/Sources/ShadowTree/`:**

- **`ShadowNodeFamily.swift`** — Move `ShadowNodeFamily` class from `ios/Sources/ReactDomNative/Bridge/ShadowNodeWrapper.swift`. This is the stable identity object shared across immutable shadow node clones. Keep the `JSManagedValue`-based `instanceHandle` property.

- **`ShadowNodeWrapper.swift`** — Move `ShadowNodeWrapper` class from `ios/Sources/ReactDomNative/Bridge/ShadowNodeWrapper.swift`. This is the `@objc` NSObject wrapper that carries props, children, family, text, and layoutFrame. Keep all clone helpers (`cloneWithNewProps`, `cloneWithNewChildren`, `cloneWithNewChildrenAndProps`, `clone`).

- **`Mutation.swift`** — Move the `Mutation` enum from `ios/Sources/ReactDomNative/Bridge/Differentiator.swift`. These are the five mutation types: create, delete, insert, remove, update.

- **`Differentiator.swift`** — Move the diff algorithm (`diff()`, `createSubtree()`, `deleteSubtree()`) from `ios/Sources/ReactDomNative/Bridge/Differentiator.swift` into this module. **Remove** the `viewRegistry` dependency and UIKit-specific `applyMutations()` method. The Differentiator in ShadowTree should be a pure function or a simple class that only produces `[Mutation]` from old/new children. It should NOT apply mutations — that responsibility moves to consumers (UIKitMutationApplier in the app, StubMutationApplier in the tester).
...
- **`ios/Sources/ReactDomNative/Bridge/Differentiator.swift`** — Replace with a `UIKitMutationApplier` class that imports `ShadowTree` and `UIKit`, takes a `ViewRegistry`, and implements the `applyMutations(_:rootView:)` method plus the `createView(for:)`, `applyProps(_:to:)`, `applyLayout(_:to:)` helpers and the `UIColor.fromCSS` extension. The app's `NativeBridge` should use `Differentiator` from `ShadowTree` for diffing, and `UIKitMutationApplier` for applying.

- **`ios/Sources/ReactDomNative/Bridge/ShadowNodeWrapper.swift`** — Replace contents with `@_exported import ShadowTree` (or just add `import ShadowTree` to files that need it) so existing code continues to compile without changes.

- **`ios/Sources/ReactDomNative/Bridge/NativeBridge.swift`** — Add `import ShadowTree`. Update to use the new `Differentiator` (from ShadowTree) and `UIKitMutationApplier` (local). The `differentiator.diff()` call stays the same. Replace `differentiator.applyMutations()` with `mutationApplier.applyMutations()`.

- **`ios/Sources/ReactDomNative/Bridge/ViewRegistry.swift`** — Add `import ShadowTree` (for `ShadowNodeFamily`). The ViewRegistry stays in the app target since it depends on UIKit.

### 2. Update `ios/Package.swift`
...
Note: The ReactDomNative target should only compile on iOS. The FantomTester target should only compile on macOS. Use conditional compilation (`#if canImport(UIKit)`) where needed, or platform-specific target settings.

### 3. Build FantomTester headless Swift binary

Create `ios/Sources/FantomTester/` with the following files:
...
- **`StubViewRegistry.swift`** — Same interface as `ViewRegistry` but maps `ShadowNodeFamily` → `StubView` instead of UIView. Template: `ios/Sources/ReactDomNative/Bridge/ViewRegistry.swift`.
...
Create `packages/fantom/src/runtime/setup.js`:
...
Create `packages/fantom/src/index.js`:
...
```javascript
const { createRoot: rendererCreateRoot } = require('../../renderer/src/renderer');
...
Create `packages/fantom/package.json`:
...
Create `packages/fantom/src/runner/`:
...
- **`jest-runner.js`** — A custom Jest runner (extends `jest-runner`). For each test file:
  ...
  class FantomRunner {
      ...
      async runTest(test) {
          ...
          // 2. Run FantomTester
          const testerBinary = path.resolve(__dirname, '../../../../ios/.build/release/FantomTester');
...
- **`bundle-test.js`** — Uses esbuild to bundle a single test file with the runtime setup prepended. Follow the pattern from `scripts/build-js.js`.
...
| File | Purpose |
|------|---------|
| `ios/Sources/ReactDomNative/Bridge/NativeBridge.swift` | Template for TesterBridge — all `$$` bridge functions |
| `ios/Sources/ReactDomNative/Bridge/ShadowNodeWrapper.swift` | Moves to ShadowTree module |
| `ios/Sources/ReactDomNative/Bridge/Differentiator.swift` | Splits: diff algo → ShadowTree, UIKit apply → UIKitMutationApplier |
| `ios/Sources/ReactDomNative/Bridge/ViewRegistry.swift` | Template for StubViewRegistry |
| `scripts/build-js.js` | esbuild config pattern for test bundler |
| `packages/renderer/src/renderer.js` | createRoot API consumed by test API |

## Output

- `ios/Sources/ShadowTree/ShadowNodeFamily.swift`
- `ios/Sources/ShadowTree/ShadowNodeWrapper.swift`
- `ios/Sources/ShadowTree/Mutation.swift`
- `ios/Sources/ShadowTree/Differentiator.swift`
- `ios/Sources/ReactDomNative/Bridge/UIKitMutationApplier.swift` (new, replaces mutation-apply code)
- `ios/Sources/ReactDomNative/Bridge/ShadowNodeWrapper.swift` (re-export only)
- `ios/Sources/ReactDomNative/Bridge/Differentiator.swift` (UIKitMutationApplier only)
- `ios/Sources/FantomTester/main.swift`
- `ios/Sources/FantomTester/TesterBridge.swift`
- `ios/Sources/FantomTester/StubView.swift`
- `ios/Sources/FantomTester/StubViewRegistry.swift`
- `ios/Sources/FantomTester/StubMutationApplier.swift`
- `ios/Package.swift` (updated)
- `packages/fantom/package.json`
- `packages/fantom/src/index.js`
- `packages/fantom/src/runtime/setup.js`
- `packages/fantom/src/runner/jest-runner.js`
- `packages/fantom/src/runner/bundle-test.js`
- `packages/fantom/src/runner/ipc.js`
